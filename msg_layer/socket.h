...
static void __exit exit_kmsg_sock(void)
{
	...
	if (transport_list_head->transport_structure == NULL && transport_list_head->next == NULL) {
		...
		//edge case of being at the end of a two node list
...
static int __init init_sock(void)
{
	...
	if (transport_list_head->transport_structure == NULL) transport_list_head->transport_stucture = transport_socket;
	else {
		trans_list = transport_list_head;
		...
		new_trans_list->transport_stucture = transport_socket;
